...
let sockaddr_of_sockaddr_datagram (addr : Eio.Net.Sockaddr.datagram) =
  match addr with `Udp a -> `Udp a

(* convert Eio.Net.Sockaddr.stream to Eio.Net.Sockaddr.t *)
let sockaddr_of_sockaddr_stream (addr : Eio.Net.Sockaddr.stream) =
  match addr with
  | `Tcp a -> `Tcp a
  | `Unix _ -> failwith "Unix sockets not supported"
