...
let provision_cert prod endpoint server_state update env ?account_key
  ...
  (* Names we've added _acme-challenge TXT records for, for cleanup in finally.
     SAN certs request one challenge per domain, so we must handle each. *)
  let acmeNames = ref [] in
  let solver =
    let add_record name record =
      ...
      let* () =
        ...
        | Ok () -> Ok ()
      in
      if not (List.exists (Domain_name.equal name) !acmeNames) then
        acmeNames := name :: !acmeNames;
  ...
  protect
    ...
    ~finally:(fun () ->
      (* once cert provisioned, remove the records *)
      List.iter
        (fun name ->
          (match
             update Domain_name.Map.empty
