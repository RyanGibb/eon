...
let zonefiles = Arg.(value & opt_all string [] & info [ "z"; "zonefile" ] ~docv:"ZONEFILE_PATHS" ~doc:"Zonefile paths.")
  ...
  List.map
    (fun ip ->
      ...
      | Ok (ip, p) ->
          ...
          (eioIp, p)
      | Error (`Msg msg) ->
          Format.fprintf Format.err_formatter "Error parsing address '%s': %s" ip msg;
...
let proto =
  ...
  let protos = [ ("tcp", [ `Tcp ]); ("udp", [ `Udp ]); ("both", [ `Tcp; `Udp ]) ] in
...
let man =
  let help_secs =
    [
      ...
      `S "LOGGING";
      `Pre
        "Log levels are defined by the LOG_LEVEL option as one of these possible values:\n\
        \    0 - No logging\n\
      ...
      `P
        "The socket(s) the server binds too can be configured with the ADDRESSES and PORT options. This allows \
         different IP addresses and ports to be used. Specifying IP addresses can be useful for restricting the \
         network interfaces to listen on, as localhost listens on all interfaces by default";
      `P
        "If IPv4-mapped IPv6 (RFC3493) is not supported, e.g. on OpenBSD, the user will need to additionally specify \
         an IPv4 address in order to serve IPv4 traffic, e.g. '-a 127.0.0.1 -a '::''.";
      `P
        "A port can be sepecified as defined in RFC4038 Section 5.1, e.g. '[::]:53' or '127.0.0.1::53', otherwise the \
         default PORT is used.";
      `P "Note that names as might be used by `getaddrinfo`, e.g. 'localhost', are not supported.";
      `S Manpage.s_bugs;
      `P "Check bug reports at https://github.com/RyanGibb/eon/issues.";
    ]
  in
  [ `S Manpage.s_description; `P "Prints help about darcs commands and other subjects…"; `Blocks help_secs ]
