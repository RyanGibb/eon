      ...
      (fun proto ->
        ...
        | `Udp ->
            fun () ->
              ...
              let sockUDP = try_bind (Eio.Net.datagram_socket ~sw ~reuse_addr:true net) (`Udp addr) in
              udp_listen sockUDP
        | `Tcp ->
            fun () ->
              Eio.Switch.run @@ fun sw ->
              let sockTCP = try_bind (Eio.Net.listen ~sw ~reuse_addr:true ~backlog:4096 net) (`Tcp addr) in
              tcp_listen sockTCP)
      proto
  in
  Eio.Fiber.all (List.flatten (List.map on_addr addrs))
