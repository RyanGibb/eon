...
let log_helper fmt (direction : dir) addr buf log_packet =
  let log_transmssion (direction : dir) addr =
    (match direction with Rx -> Format.fprintf fmt "<-" | Tx -> Format.fprintf fmt "->");
    Format.print_space ();
...
let level_1 fmt (direction : dir) addr buf =
  let log_packet (packet : Dns.Packet.t) =
    ...
    Format.fprintf fmt "header %04X question %a@ data %a@" id Dns.Packet.Question.pp packet.question Dns.Packet.pp_data
      packet.data
  in
...
let get i = match i with Level0 -> level_0 | Level1 -> level_1 | Level2 -> level_2 | Level3 -> level_3
